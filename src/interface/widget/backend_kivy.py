...
from kivy.graphics import (Color, Line, Mesh, Rectangle, Rotate, StencilPop,
...
from kivy.uix.actionbar import (ActionBar, ActionButton, ActionOverflow,
                                ActionPrevious, ActionSeparator,
                                ActionToggleButton, ActionView)
...
from matplotlib.backend_bases import (Event, FigureCanvasBase,
...
class RendererKivy(RendererBase):
  ...
  def draw_path_collection(self, gc, master_transform, paths, all_transforms,
    ...
    uses_per_path = self._iter_collection_uses_per_path(paths, all_transforms,
                                                        offsets, facecolors,
                                                        edgecolors)
    ...
    if not should_do_optimization:
      return RendererBase.draw_path_collection(self, gc, master_transform,
                                               paths, all_transforms, offsets,
    ...
    for xo, yo, path_poly, gc0, rgbFace in self._iter_collection(
        gc, master_transform, all_transforms, path_codes, offsets, offsetTrans,
        facecolors, edgecolors, linewidths, linestyles, antialiaseds, urls,
        offset_position):
      list_canvas_instruction = self.get_path_instructions(gc0,
                                                           path_poly,
                                                           closed=True,
                                                           rgbFace=rgbFace)
  ...
  def _convert_path(self,
    ...
    if clip:
      ...
      clip = None
    if _mpl_ge_1_5:
      return _path.convert_to_string(path, transform, clip, simplify, sketch, 6,
                                     [b'M', b'L', b'Q', b'C', b'z'],
                                     False).decode('ascii')
    else:
...
class NavigationToolbar2Kivy(NavigationToolbar2):
  ...
  def draw_rubberband(self, event, x0, y0, x1, y1):
    ...
    h = abs(y1 - y0)
    rect = [
        int(val) for val in (min(x0, x1) + self.canvas.x,
                             min(y0, y1) + self.canvas.y, w, h)
    ]
...
class TimerKivy(TimerBase):
  ...
  def _timer_start(self):
    ...
    # never be stopped.
    self._timer_stop()
    self._timer = Clock.schedule_interval(self._on_timer,
