...
parser = argparse.ArgumentParser(prog="stitching_detailed.py",
...
parser.add_argument(
    ...
    help="Type of features used for images matching. The default is '%s'." %
...
parser.add_argument(
    '--estimator',
    action='store',
    default=list(ESTIMATOR_CHOICES.keys())[0],
    help=
    "Type of estimator used for transformation estimation. The default is '%s'."
...
parser.add_argument(
    ...
    action='store',
    help=
    "Confidence for feature matching step. The default is 0.3 for ORB and 0.65 for other feature types.",
    type=float,
    dest='match_conf')
parser.add_argument(
    '--conf_thresh',
...
parser.add_argument(
    '--ba',
    action='store',
    default=list(BA_COST_CHOICES.keys())[0],
    help="Bundle adjustment cost function. The default is '%s'." %
...
parser.add_argument(
    ...
    type=str,
    dest='ba_refine_mask')
parser.add_argument('--wave_correct',
                    action='store',
                    default=WAVE_CORRECT_CHOICES[0],
                    help="Perform wave effect correction. The default is '%s'" %
                    WAVE_CORRECT_CHOICES[0],
...
parser.add_argument('--warp',
                    action='store',
                    default=WARP_CHOICES[0],
                    help="Warp surface type. The default is '%s'." %
                    WARP_CHOICES[0],
...
parser.add_argument('--seam',
                    action='store',
                    default=list(SEAM_FIND_CHOICES.keys())[0],
                    help="Seam estimation method. The default is '%s'." %
...
parser.add_argument('--expos_comp',
                    action='store',
                    default=list(EXPOS_COMP_CHOICES.keys())[0],
                    help="Exposure compensation method. The default is '%s'." %
                    list(EXPOS_COMP_CHOICES.keys())[0],
                    choices=EXPOS_COMP_CHOICES.keys(),
                    type=str,
                    dest='expos_comp')
parser.add_argument('--expos_comp_nr_feeds',
                    action='store',
                    default=1,
                    help="Number of exposure compensation feed.",
                    type=np.int32,
                    dest='expos_comp_nr_feeds')
parser.add_argument(
...
parser.add_argument('--blend',
                    action='store',
                    default=BLEND_CHOICES[0],
                    help="Blending method. The default is '%s'." %
                    BLEND_CHOICES[0],
...
def main(img_names):
  ...
  for name in img_names:
    ...
    if is_seam_scale_set is False:
      seam_scale = min(
          1.0,
          np.sqrt(seam_megapix * 1e6 / (full_img.shape[0] * full_img.shape[1])))
  ...
  for idx in range(0, num_images):
    ...
    p, mask_wp = warper.warp(masks[idx], K, cameras[idx].R, cv.INTER_NEAREST,
                             cv.BORDER_CONSTANT)
  ...
  for idx, name in enumerate(img_names):
    ...
    K = cameras[idx].K().astype(np.float32)
    corner, image_warped = warper.warp(img, K, cameras[idx].R, cv.INTER_LINEAR,
                                       cv.BORDER_REFLECT)
    ...
    seam_mask = cv.resize(dilated_mask,
                          (mask_warped.shape[1], mask_warped.shape[0]), 0, 0,
    ...
    if timelapse:
      ma_tones = np.ones((image_warped_s.shape[0], image_warped_s.shape[1]),
                         np.uint8)
      ...
      if pos_s == -1:
        ...
        fixed_file_name = img_names[idx][:pos_s +
                                         1] + "fixed_" + img_names[idx][pos_s +
                                                                        1:]
...
if __name__ == '__main__':
  print(__doc__)
  # images = [(r'D:\repo\ThermalImage\Panorama\KICT\
  #     WaterResourceBuilding\vis\preprocess\IR_2020-10-19_0007_ir.jpg'),
  #           (r'D:\repo\ThermalImage\Panorama\KICT\
  #     WaterResourceBuilding\vis\preprocess\IR_2020-10-19_0008_ir.jpg')]
  # data_dir = r'D:\repo\ThermalImage\Panorama\KICT\WaterResourceBuilding\vis\preprocess'
  #
  # images = ['IR_2020-10-19_0009.png', 'IR_2020-10-19_0012.png']
  # images = ['IR_2020-10-19_0007.png', 'IR_2020-10-19_0008.png']

  data_dir = r'D:\Program\opencv_extra\testdata\stitching'

  images = ['a1.png', 'a2.png', 'a3.png']
  images = ['newspaper{}.jpg'.format(x + 1) for x in range(4)]

  images = [data_dir + '\\' + x for x in images]

  # from pathlib import Path
  # data_dir = Path(
  #     r'D:\repo\ThermalImage\Panorama\KICT\WaterResourceBuilding\ir\preprocess'
  # )
  # # images = [x.as_posix() for x in data_dir.rglob('*.png')]
  # images = [
  #     data_dir.joinpath('IR_2020-10-19_{:04d}.png'.format(x))
  #     for x in [7, 9, 10, 8, 11]
  # ]
  # assert all(x.exists() for x in images)
  # images = [x.as_posix() for x in images]

  main(img_names=images)
